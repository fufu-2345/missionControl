...
- **ถ้าข้อมูลไม่ครบ → ถามก่อน อย่าเดาเอง:**
  ...
  - **ชื่อโปรเจกต์ `<ชื่อ>`** → เสนอชื่อจาก requirement แล้วให้ผู้ใช้ยืนยัน/แก้ (ชื่อนี้คุม folder `soulbrew/project/<ชื่อ>/` + team `orches-<ชื่อ>`)
...
- โปรเจกต์ใหม่ใต้ **`soulbrew/project/<ชื่อ>/` เสมอ** (contents กันออกจาก soulbrew git ด้วย `project/.gitignore`)
...
- ชื่อโปรเจกต์ + folder `soulbrew/project/<ชื่อ>/` + **team ที่เลือก** (ใหม่ `orches-<ชื่อ>` / ตั้งเอง / reuse เดิม)
...
1. **`oracle_trace`** (เสมอ): `query`=สรุปงาน · `project`=`soulbrew/project/<ชื่อ>` · `scope`=project · `foundFiles`+`matchReason`+`confidence` · `agentCount`/`durationMs`
