...
const _script = [
  ...
  "    if(!v.oracleOnline){ el.innerHTML = '<h2>Search / Oracle</h2><div class=\"so-off\">Oracle offline (:47778) — เปิด oracle server ก่อนถึงจะตั้งค่าได้ <button class=\"so-btn\" data-so=\"retry\">ลองใหม่</button></div>'; return; }",
  ...
  "    else if(act==='retry') post('reloadSearch');",
  "  });",
